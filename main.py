...
async def check_fssp(last: str, first: str, middle: str, dob: str, region: int):
    try:
        # В API newdb: firstname = фамилия, lastname = имя (их специфика из документации)
        params = {
            "method": "fssp_person",
            "firstname": last,
            "lastname": first,
        ...
        # Регион — только если явно выбран (0 ломает запрос)
        ...
        print(f"[FSSP] Ответ state={data.get('state')}, balance={data.get('balance')}")
...
@app.get("/")
def root():
    return {"status": "ok", "service": "Person Check API v5"}
