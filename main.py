...
КРИТИЧНО: если в разделе «СЦЕНАРНЫЕ ФЛАГИ» НЕТ упоминания несовершеннолетнего — \
КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать о несовершеннолетних собственниках, органах опеки, \
разрешении опеки. Слово «несовершеннолетний» в справочных пунктах чеклиста (форма 9, \
долевая собственность) — это общие советы, а не факт о конкретном собственнике. \
Не путай справочную информацию с реальными данными проверки. \
...
def build_deepseek_user_prompt(owner: OwnerRequest, checklist: list, scoring: dict, recs: list) -> str:
    ...
    # Несовершеннолетний — проверяем ТОЛЬКО реальных владельцев из запроса,
    # а не текст чеклиста (иначе слово «несовершеннолетний» из справочных пунктов
    # вызывает галлюцинацию о несуществующем ребёнке-собственнике)
    has_minor_owner = any(
        (o.is_minor is True) or (o.role == "minor")
        for o in (req.owners or [])
    )
    if has_minor_owner:
        scenario_flags.append("среди собственников есть несовершеннолетний — требуется разрешение органов опеки")
